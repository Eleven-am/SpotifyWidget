import {LiveFactory, html, LiveEvent} from "@eleven-am/pondlive";
...
import {monitorChannel} from "./channels";
    ...
    return html`
        <div class="${ss.widget}">
            ...
            <div class="${ss.widgetSvgHolder}">
                <svg class="${ss.widgetSvg}" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
...
const ProgressBar = (ss: Record<string, string>, elapsed?: string, remaining?: string) => {
    return html`
...
export const Player = LiveFactory<LiveContext>({
    routes: [],
    ...
    async onRendered(socket, router) {
        const token = this.channel;
        if (!token)
            return router.navigateTo('/login');

        monitorChannel.subscribe(socket);
        const channel = monitorChannel.channelData[this.channel];

        if (!channel) {
            const monitor = new Monitor(token, monitorChannel, userClass);
            monitorChannel.assign({
                [this.channel]: {monitor, users: 1}
            })
            monitor.monitorClient();

        } else  {
            const message = await channel.monitor.broadcast();
            monitorChannel.assign({
                [this.channel]: {
                    monitor: channel.monitor,
                    users: channel.users + 1
                }
            })
    ...
    async onEvent(event: LiveEvent<ClientEvent>, _socket, _router) {
        const channel = monitorChannel.channelData[this.channel];
    ...
    manageStyles(css) {
        ...
        return css`
          ...
          .infoHolderSvg {
            ...
            margin-left: 1vw;
            padding: 0;
            visibility: ${this.state?.track?.explicit ? 'visible': 'hidden'};
          }
          ...
          .progressBarFill {
            width: ${(this.state?.details?.progressPercent || 0) * 100}%;
            height: 100%;
            background: #1ed760;
            border-radius: 1vw;
            position: absolute;
    ...
    onInfo(info, socket, router) {
        monitorChannel.handleInfo(info, payload => {
            if (payload.userId === this.channel) {
                if (payload.error) {
                    console.log(payload.error);
                    return router.flashMessage = payload.error;
                }

                if (payload.message) {
                    socket.assign({state: payload.message});
                    if (this.state?.track?.name !== payload.message.track?.name) {
                        if (payload.message.track)
                            router.pageTitle = `${payload.message.track?.name} - ${payload.message?.track.artists[0].name}`;
    ...
    render(_renderRoutes, classes ) {
        return html`
            <div class="${classes.holder}">
                ${Widget(classes, this.state?.track?.album?.images[0].url)}
                <div class="${classes.info}">
                    ${TrackInfo({device: this.state?.device, track: this.state?.track}, classes)}
                    ${Buttons(classes, {state: this.state?.details?.playerState, device: this.state?.device})}
                    ${ProgressBar(classes, this.state?.details?.elapsed, this.state?.details?.remains)}
                </div>
            </div>`
    },
    ...
    onUnmount() {
        ...
        const monitor = channel.monitor as Monitor;
        if (channel.users === 1) {
