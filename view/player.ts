...
type InfoEvent = MonitorMessage & {error?: string}
...
export const Player = LiveFactory<LiveContext>({
    ...
    async onEvent(event: ClientEvent, context, socket, _router) {
        const token = context.channel;
        const channel = socket.getChannel(token) as PondLiveChannel;
    ...
    onInfo(info: InfoEvent, context, socket, router) {
        if (info && info?.error) {
            console.log(info.error);
            router.flashMessage = info.error;
        }
